-- Migration: create module tables for Aviculture, Agriculture, Cuniculture, Apiculture, Bovins and Pêche
-- Creates: poulaillers, parcelles, clapiers, ruchers, etables, zones_peche
...
CREATE TABLE IF NOT EXISTS public.zones_peche (
  ...
  tenant_id uuid REFERENCES public.tenants(id) ON DELETE CASCADE,
  name text NOT NULL,
  geometry jsonb DEFAULT '{}'::jsonb, -- optional geo/info
  metadata jsonb DEFAULT '{}'::jsonb,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
...
-- Optionally: simple cohort/troupeau tables (can be extended later)
...
-- Trigger to update updated_at column on modification (generic)
