...
const ZonesPecheList: React.FC = () => {
  ...
  const { items, loading, error, create, update, remove } = useZonesPeche();
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-medium">Zones de pêche</h3>
        <div className="flex gap-2">
          <Button onClick={() => { setSelected(null); setOpen(true); }}>Créer une zone</Button>
        </div>
      </div>
      <div className="bg-white rounded-lg shadow p-4">
        {loading ? <div className="text-sm text-muted-foreground">Chargement...</div> : items.length === 0 ? (
          <div className="text-sm text-muted-foreground">Aucune zone de pêche.</div>
        ) : (
