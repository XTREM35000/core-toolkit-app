...
import RucherModal from './RucherModal';
...
const RuchersList: React.FC = () => {
  ...
  const { items, loading, error, create, update, remove } = useRuchers();
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-medium">Ruchers</h3>
        <div className="flex gap-2">
          <Button onClick={() => { setSelected(null); setOpen(true); }}>Créer un rucher</Button>
        </div>
      </div>
      <div className="bg-white rounded-lg shadow p-4">
        {loading ? <div className="text-sm text-muted-foreground">Chargement...</div> : items.length === 0 ? (
          <div className="text-sm text-muted-foreground">Aucun rucher.</div>
        ) : (
      ...
      <RucherModal open={open} onOpenChange={(v) => { if (!v) setSelected(null); setOpen(v); }} rucher={selected} onSaved={async (item) => { await handleSaved(item); setOpen(false); }} />
    </div>
  );
};
