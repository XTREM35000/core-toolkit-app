...
export async function selectAll<T = any>(table: any, orderBy?: { column: string; ascending?: boolean }) {
...
export async function insertOne<T = any>(table: any, payload: Partial<T>) {
  const res = await supabase.from(table).insert(payload).select();
  return res as { data: T[] | null; error: any };
}

export async function updateOne<T = any>(table: any, id: string, payload: Partial<T>) {
  const res = await supabase.from(table).update(payload).eq('id', id).select();
  return res as { data: T[] | null; error: any };
}

export async function deleteOne(table: any, id: string) {
